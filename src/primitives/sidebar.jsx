 ...
 * @property {(open: boolean) => void} setOpenMobile
...
function SidebarProvider({
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('fold_sidebar')
      if (saved !== null) {
        const isFolded = saved === 'true'
        _setOpen(!isFolded) // fold_sidebar = true means collapsed, so open = false
      }
    }
  }, [])
  ...
  const setOpen = React.useCallback(
    (value) => {
      ...
      if (typeof window !== 'undefined') {
        const isFolded = !openState // open = false means folded = true
        localStorage.setItem('fold_sidebar', isFolded.toString())
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex min-h-svh w-full max-w-full overflow-hidden",
            className
          )}
          {...props}
        >
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ? "p-2 group-data-[collapsible=icon]:w-18"
...
/**
 * @param {React.ComponentProps<"main">} props
 */
function SidebarInset({ className, ...props }) {
  return (
    <main
      ...
      className={cn(
        "bg-background relative flex w-full flex-1 flex-col max-w-full overflow-hidden",
...
function SidebarSeparator({
  ...
  return (
    <Separator
      ...
      className={cn("bg-sidebar-border mx-0 w-full", className)}
      {...props}
    />
  )
}
...
/**
 * @param {React.HTMLAttributes<HTMLDivElement> & { asChild?: boolean }} props
 */
...
/**
 * @param {React.HTMLAttributes<HTMLLIElement>} props
 */
...
/**
 * @param {React.HTMLAttributes<HTMLDivElement> & { showIcon?: boolean }} props
 */
